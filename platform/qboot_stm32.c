...
rt_weak void qbt_jump_to_app(void)
{
    ...
    rt_uint32_t stk_addr = 0;
    rt_uint32_t reset_handler = 0;
    const rt_uint8_t send[4] = { 0x03, 0x00, 0x00, 0x00 };
    rt_uint8_t recv[8] = { 0 };
    ...
    stk_addr = recv[0];
    stk_addr += ((rt_uint32_t)recv[1] << 8);
    stk_addr += ((rt_uint32_t)recv[2] << 16);
    stk_addr += ((rt_uint32_t)recv[3] << 24);

    reset_handler = recv[4];
    reset_handler += ((rt_uint32_t)recv[5] << 8);
    reset_handler += ((rt_uint32_t)recv[6] << 16);
    reset_handler += ((rt_uint32_t)recv[7] << 24);
...
rt_weak void qbt_jump_to_app(void)
{
    ...
    if ((((rt_uint32_t)app_func & 0xff000000) != 0x08000000) || (((stk_addr & 0x2ff00000) != 0x20000000) && ((stk_addr & 0x2ff00000) != 0x24000000)))
    {
        LOG_E("No legitimate application.");
        return;
    }
