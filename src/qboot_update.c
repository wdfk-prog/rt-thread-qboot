...
rt_bool_t qbt_update_mgr_download_begin(void)
{
    ...
    {
        rt_err_t cleanup_result = qbt_update_mgr_download_cleanup();
        if (cleanup_result != RT_EOK && qbt_update_mgr_ops_ready())
        {
            s_mgr.ops->on_error(cleanup_result);
        }
        return RT_FALSE;
    }
...
void qbt_update_mgr_on_abort(void)
{
    ...
    {
        s_mgr.ready_flag = RT_FALSE;
        qbt_update_mgr_set_state(QBT_UPD_STATE_ERROR);
        s_mgr.ops->on_error(result);
    }
#endif /* QBOOT_UPDATE_MGR_USE_DOWNLOAD_HELPER */
