...
out_dir="$host_root/sanitizer"
mkdir -p "$out_dir"
log_file="$out_dir/asan-ubsan.log"

QBOOT_HOST_OUT_DIR="$host_root" QBOOT_HOST_SANITIZER=1 bash .github/ci/qboot/build-host-sim.sh > "$out_dir/build.log" 2>&1
ASAN_OPTIONS="detect_leaks=1:halt_on_error=1" \
UBSAN_OPTIONS="halt_on_error=1" \
QBOOT_HOST_OUT_DIR="$host_root" bash .github/ci/qboot/run-host-sim.sh > "$log_file" 2>&1
