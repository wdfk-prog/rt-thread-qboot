...
run_extra_hpatch_release() {
  ...
  case_count=$((case_count + 1))
  if [ "$expect" = "fail" ]; then
    if run_named_release_case custom-hpatch-only "$case_name" "$pkg" "$fixture_dir/old_app.bin" "$new_img" "$log_file" --expect-receive 1 --expect-first-success 0 --expect-success 0 --expect-jump 0 --expect-sign 0 --expect-app old "${extra_args[@]}"; then
      pass_count=$((pass_count + 1))
      printf 'QBOOT_HOST_CASE_PASS %s\n' "$case_name"
      printf '| custom-hpatch-only | policy-smoke | %s | PASS | - | - | - | - | - | - | - | release | `%s` | additional HPatchLite rejection smoke coverage |\n' "$case_name" "$log_file" >> "$summary"
    else
      printf 'QBOOT_HOST_CASE_FAIL %s\n' "$case_name"; cat "$log_file"; exit 1
    fi
  else
    if run_named_release_case custom-hpatch-only "$case_name" "$pkg" "$fixture_dir/old_app.bin" "$new_img" "$log_file" --expect-receive 1 --expect-first-success 1 --expect-success 1 --expect-jump 1 --expect-sign 1 --expect-app new "${extra_args[@]}"; then
      pass_count=$((pass_count + 1))
      printf 'QBOOT_HOST_CASE_PASS %s\n' "$case_name"
      printf '| custom-hpatch-only | policy-smoke | %s | PASS | - | - | - | - | - | - | - | release | `%s` | additional HPatchLite success smoke coverage |\n' "$case_name" "$log_file" >> "$summary"
    else
      printf 'QBOOT_HOST_CASE_FAIL %s\n' "$case_name"; cat "$log_file"; exit 1
    fi
  fi
...
run_extra_hpatch_release stream-hpatch-full-diff-package "$fixture_dir/custom-hpatch-host-full-diff.rbl" "$fixture_dir/hpatch_new_app.bin" success --chunk 257
...
run_extra_hpatch_release resource-hpatch-malloc-fail-smoke "$fixture_dir/custom-hpatch-host-full-diff.rbl" "$fixture_dir/hpatch_new_app.bin" fail --malloc-fail-after 0
