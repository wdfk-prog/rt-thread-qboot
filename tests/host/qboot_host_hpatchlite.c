                                     ...
                                     int patch_file_offset)
{
    ...
    output = patch + off;
    off += diff_size;
    /* output + new_pos and patch + off intentionally alias the same bytes.
     * restore_cover_from_old() reads sub_diff[i] before writing cover_out[i],
     * so keep this path strictly sequential.
     */
