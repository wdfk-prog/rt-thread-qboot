...
/**
 * @brief Check whether a host receive mode uses strict protocol simulation.
 *
 * @param mode Receive mode string.
 *
 * @return RT_TRUE for strict protocol-adapter modes, RT_FALSE otherwise.
 */
static rt_bool_t qboot_host_receive_mode_is_protocol(const char *mode)
{
    return (rt_strcmp(mode, "protocol-total-size-short") == 0 ||
...
/**
 * @brief Validate one packet against strict protocol receive state.
 *
 * @param base           Original transfer buffer used by the host fixture.
 * @param expected_off   Next expected contiguous byte offset.
 * @param declared_total Protocol-declared transfer length.
 * @param packet_off     Packet offset from transport metadata.
 * @param packet_data    Packet payload bytes.
 * @param packet_size    Packet payload size in bytes.
 * @param allow_duplicate RT_TRUE to allow identical duplicate packets.
 *
 * @return RT_TRUE when the packet is protocol-valid, RT_FALSE otherwise.
 */
static rt_bool_t qboot_host_protocol_packet_valid(const rt_uint8_t *base,
                                                  rt_uint32_t expected_off,
                                                  rt_uint32_t declared_total,
                                                  rt_uint32_t packet_off,
                                                  const rt_uint8_t *packet_data,
                                                  rt_uint32_t packet_size,
                                                  rt_bool_t allow_duplicate)
{
    if (packet_off > expected_off || expected_off > declared_total ||
        packet_size > (declared_total - packet_off))
    {
        return RT_FALSE;
    }
    if (packet_off < expected_off)
    {
        rt_uint32_t accepted_len = expected_off - packet_off;
        if (!allow_duplicate || accepted_len < packet_size)
        {
            return RT_FALSE;
        }
        return (rt_memcmp(base + packet_off, packet_data, packet_size) == 0) ?
                   RT_TRUE : RT_FALSE;
    }
    return (packet_size <= (declared_total - expected_off)) ? RT_TRUE : RT_FALSE;
}

/**
 * @brief Receive package bytes through a strict host protocol adapter.
 *
 * This simulation exercises the real helper download session while keeping
 * transport-level declared length, offset ordering, gap, overlap, and duplicate
 * policy outside qbt_update_mgr_download_write(). Invalid protocol metadata
 * aborts the active session through qbt_update_mgr_download_finish(RT_FALSE).
 *
 * @param data       Package bytes.
 * @param size       Available package byte count.
 * @param chunk_size Maximum chunk size per write.
 * @param mode       Strict protocol negative test mode.
 *
 * @return RT_TRUE when the strict protocol accepts the transfer,
 *         RT_FALSE otherwise.
 */
static rt_bool_t qboot_host_receive_download_protocol(const rt_uint8_t *data,
                                                      rt_uint32_t size,
                                                      rt_uint32_t chunk_size,
                                                      const char *mode)
{
    rt_uint32_t declared_total = size;
    rt_uint32_t expected_off = 0u;
    rt_uint32_t pos = 0u;
    rt_bool_t active = RT_FALSE;
    rt_bool_t ok = RT_FALSE;
    rt_bool_t resume_after_reset = RT_FALSE;
    rt_bool_t resume_triggered = RT_FALSE;

    if (rt_strcmp(mode, "protocol-total-size-short") == 0)
    {
        declared_total = size - 1u;
    }
    else if (rt_strcmp(mode, "protocol-total-size-long") == 0)
    {
        if (size == (rt_uint32_t)-1)
        {
            return RT_FALSE;
        }
        declared_total = size + 1u;
    }
    else if (rt_strcmp(mode, "protocol-resume-after-reset") == 0)
    {
        resume_after_reset = RT_TRUE;
    }
    ...
    while (pos < size)
    {
        ...
        rt_uint32_t packet_off = pos;

        if (chunk > chunk_size)
        {
            chunk = chunk_size;
        }
        if (rt_strcmp(mode, "protocol-gap-rejected") == 0 && pos >= chunk_size)
        {
            packet_off = pos + 1u;
        }
        else if (rt_strcmp(mode, "protocol-overlap-rejected") == 0 &&
                 pos >= chunk_size)
        {
            packet_off = pos - 1u;
        }

        if (!qboot_host_protocol_packet_valid(data, expected_off, declared_total,
                                               packet_off, data + pos, chunk,
                                               RT_FALSE))
        {
            goto exit;
        }
        if (!qbt_update_mgr_download_write(packet_off, (rt_uint8_t *)(data + pos),
                                           chunk))
        {
            active = RT_FALSE;
            goto exit;
        ...
        if (resume_after_reset && !resume_triggered && pos < size)
        {
            /* Simulate a protocol adapter reset after a partial transfer. The
             * download helper session stays open, while the adapter resumes
             * with the persisted next contiguous offset instead of restarting
             * from zero.
             */
            expected_off = pos;
            resume_triggered = RT_TRUE;
        }

        if (rt_strcmp(mode, "protocol-duplicate-different-data") == 0)
        {
            rt_uint8_t duplicate_byte = (rt_uint8_t)(data[0] ^ 0x5Au);
            if (!qboot_host_protocol_packet_valid(data, expected_off,
                                                   declared_total, 0u,
                                                   &duplicate_byte, 1u,
                                                   RT_TRUE))
            {
            ...
            if (!qbt_update_mgr_download_write(0u, &duplicate_byte, 1u))
            {
                active = RT_FALSE;
                goto exit;
            }
    ...
    if (expected_off == declared_total)
    {
        ok = (qbt_update_mgr_download_finish(RT_TRUE) == RT_EOK) ?
                 RT_TRUE : RT_FALSE;
        active = RT_FALSE;
    }
                                           ...
                                           const char *mode)
{
    ...
    if (qboot_host_receive_mode_is_protocol(mode))
    {
        return qboot_host_receive_download_protocol(data, size, chunk_size, mode);
    }
    if (!qbt_update_mgr_download_begin())
    {
        return RT_FALSE;
