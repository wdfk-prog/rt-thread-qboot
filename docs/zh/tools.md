...
注意：当 `--cmprs hpatchlite` 时，仍要求 `--crypt none`；固件侧通过编译期检查互斥 `QBOOT_USING_AES` 和 `QBOOT_USING_HPATCHLITE`，因此会拒绝加密 HPatchLite 包。HPatchLite 包仍允许使用 `--algo2 crc`：源包流式检查阶段会跳过 HPatchLite raw CRC 校验，目标分区校验阶段仍会校验还原后的 APP。
...
4. **生成 HPatchLite 差分包**：读取旧固件和新固件，生成 HPatchLite full-diff patch body，差分包体可选择不压缩或使用 HPatchLite 自带的 `_CompressPlugin_tuz` 压缩，外层 RBL 加密固定为 `none`，然后封装为 RBL 包。
...
- `fastlz`：使用 literal-only level-1 records 生成并解析 QBoot FastLZ block stream，并按运行时输出缓冲大小拆分 raw block
- `quicklz`：使用 level-1 stored packets 生成并解析 QBoot QuickLZ block stream，并按运行时输出缓冲大小拆分 raw block
...
约束：CI 只验证 QBoot 行为、工具输出和 runtime codec 集成。不要新增仅验证 CI harness
自身、artifact 清单、脚本生成物或中间日志是否存在的一类自测；这类检查会让 CI
验证自己的脚手架，而不是验证 QBoot 发布行为。

- 对全部受支持的 `--crypt`、`--cmprs`、`--algo2` 组合，对比手动包体模式与 `tools/package_tool.py` 的 `.rbl` 完整字节流；AES/XOR 与 `hpatchlite` 的组合作为明确拒绝路径覆盖。
- 对浏览器端真实处理路径执行回归测试，包括 RBL header 解析、CRC 校验、gzip/FastLZ/QuickLZ 打包与还原、AES 测试向量、AES 打包/还原、HPatchLite 差分往返，以及无效组合的明确失败路径。
- host runtime codec 集成：
  - 拉取 crclib、tinycrypt、zlib、QuickLZ、FastLZ、HPatchLite 外部源码，并分别与支持 AES 的 runner 和 HPatchLite-only QBoot C host runner 链接。
  - 通过互斥的 AES runner 和 HPatchLite runner 对 none、gzip、AES、AES+gzip、QuickLZ、FastLZ、HPatchLite golden fixture 执行 release。
  - crclib 和 TinyCrypt 软件包源码树会被准备，并通过生成的 runtime-only ABI adapter 编译。
  - adapter 使用符号重命名包含外部实现文件，然后导出 QBoot host 需要的 CRC/AES 符号。
  - 兼容头只用于适配 include 名称，不链接 host codec stub。
  - runtime 脚本把实际选中的外部源码写入 source manifest；如果软件包没有提供必要运行时符号，会在链接前失败。
  - 该层验证 C runtime 集成路径；真实 Flash 时序和跳转副作用仍需要板级/HIL 测试。
...
网页差分模式会生成 HPatchLite full-diff 原生格式包体，输入为旧固件和新固件；应用差分包时需要旧固件和 `.rbl` 包。RBL 外层 `cmprs` 字段固定为 `hpatchlite`，外层 `crypt` 字段固定为 `none`，不能选择 `none`/`gzip`/`quicklz`/`fastlz` 作为外层压缩算法。差分包体内部可选择不压缩或使用 HPatchLite 自带的 `_CompressPlugin_tuz` 压缩/解压，不会走 RBL 外层 `gzip`/`zlib`，也不会使用通用 raw-deflate。该实现会把完整新固件作为 diff data 写入 HPatchLite patch，因此格式兼容 QBoot HPatchLite patch flow，但不会获得原生 `hdiffi` 相似块匹配带来的体积压缩。生产环境如需紧凑差分包，仍建议使用原生 HPatchLite 工具生成包体后走“手动包体打包”。
