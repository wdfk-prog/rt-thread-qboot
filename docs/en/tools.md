...
Note: when `--cmprs hpatchlite` is selected, `--crypt none` is still required; encrypted HPatchLite packages are rejected because firmware builds make `QBOOT_USING_AES` and `QBOOT_USING_HPATCHLITE` mutually exclusive. `--algo2 crc` remains valid for HPatchLite packages: source package streaming skips HPatchLite raw CRC verification, while destination partition verification still checks the restored APP image.
...
4. **Build HPatchLite differential package**: reads old and new firmware, creates an HPatchLite full-diff patch body, optionally leaves that body uncompressed or compresses it through HPatchLite's own `_CompressPlugin_tuz` path, forces outer RBL encryption to `none`, and wraps it as an RBL package.
...
- `fastlz`: QBoot FastLZ block-stream encoding and decoding using literal-only level-1 records, split so each raw block fits the runtime output buffer
- `quicklz`: QBoot QuickLZ block-stream encoding and decoding using level-1 stored packets, split so each raw block fits the runtime output buffer
...
Constraint: CI should validate QBoot behavior, tool output, and runtime codec integration.
Do not add checks whose only purpose is to validate the CI harness itself, artifact
inventories, generated bookkeeping, or intermediate log presence; those checks make CI
validate its own scaffolding rather than QBoot release behavior.

- Across every supported `--crypt`, `--cmprs`, and `--algo2` combination, prepared-body mode is compared byte-for-byte with `tools/package_tool.py`; AES/XOR with `hpatchlite` is covered as an explicit rejection path.
...
- Host runtime codec integration:
  - Fetches external crclib, TinyCrypt, zlib, QuickLZ, FastLZ, and HPatchLite sources and links them with mutually exclusive AES-capable and HPatchLite-only QBoot C host runners.
  - Releases golden fixtures for none, gzip, AES, AES+gzip, QuickLZ, FastLZ, and HPatchLite through mutually exclusive AES and HPatchLite runners.
  - Stages crclib and TinyCrypt package trees, then compiles them through a generated runtime-only ABI adapter with symbol renaming.
  - Exports the QBoot host CRC/AES symbols; compatibility headers only adapt include names, and no host codec stubs are linked.
  - Records selected external source files in a source manifest and fails before linking if a package lacks a required runtime symbol.
  - Verifies the C runtime integration path; board/HIL tests are still required for Flash timing and jump-side effects.
