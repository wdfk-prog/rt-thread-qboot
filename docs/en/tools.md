...
Note: when `--cmprs hpatchlite` is selected, the script forces `algo2` to `none` and requires `--crypt none`; encrypted HPatchLite packages are rejected because firmware builds make `QBOOT_USING_AES` and `QBOOT_USING_HPATCHLITE` mutually exclusive.
